...
const BreadcrumbItem = memo(
  ({ url, title, isLast = false, showIcon = true }: BreadcrumbItemProps) => {
    ...
    return (
      <li
        className={`${styles.breadcrumbItem} ${isLast ? styles.lastItem : ""}`}
      >
        {showIcon && SvgIcon}
        {isLast ? (
          <span className={styles.breadcrumbTitle}>{title}</span>
        ) : (
          <Link to={url} className={styles.breadcrumbLink}>
            {title}
          </Link>
        )}
      </li>
